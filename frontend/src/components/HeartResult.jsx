...
const HeartResultModal = () => {
  ...
  // Parse risk score from backend with a heuristic fallback if the API fails
  ...
  // Medical Safety Formatting ("0%" mathematically converted to "< 1%")
  const displayRiskScore = (prediction && finalRiskScore < 1) ? "< 1%" : `${finalRiskScore}%`;
  ...
  const createPDFDocument = async () => {
    ...
    // PAGE 0: Full-Bleed Cover Page
    if (coverImg) {
      doc.addImage(coverImg, "PNG", 0, 0, 210, 297);
      doc.addPage();
    }
    
    // PAGE 1: Medical Parameters & Diagnosis
    doc.setFont("helvetica", "bold");
    doc.setTextColor(0, 51, 102);
    doc.setFontSize(24);
    doc.text("CARDIOLOGY REPORT", 105, 20, null, "center");
    ...
    if (form) {
      const validEntries = Object.entries(form).filter(([key]) => parameterNames[key]);
      
      for (let i = 0; i < validEntries.length; i += 2) {
        // Column 1
        const [k1, v1] = validEntries[i];
        doc.setFont("helvetica", "bold");
        doc.text(`${parameterNames[k1]}:`, 20, yPos);
        doc.setFont("helvetica", "normal");
        doc.text(String(formatValue(k1, v1)), 65, yPos);
        
        // Column 2 (if exists)
        if (i + 1 < validEntries.length) {
          const [k2, v2] = validEntries[i + 1];
          doc.setFont("helvetica", "bold");
          doc.text(`${parameterNames[k2]}:`, 110, yPos);
          doc.setFont("helvetica", "normal");
          doc.text(String(formatValue(k2, v2)), 150, yPos);
        }
    ...
    doc.setFontSize(12);
    doc.text(`Total Calculated Risk: ${displayRiskScore}`, 20, yPos + 10);

    // Render footer logo on Page 1
    if (logoImg) doc.addImage(logoImg, "PNG", 95, 265, 20, 20);

    // PAGE 2: Guidelines and Avoids
    doc.addPage();
    let p2Y = 20;
    ...
    // Helper for rendering wrapped bullet point text cleanly
    ...
    // Single Vertical Column layout for Avoids to prevent clipping
    const startX = 25;
      ...
      <motion.div style={overlay}>
        <div style={card}>
          ...
          >
            ...
            <p style={{ fontSize: 18 }}>Estimated Total Risk: <b>{displayRiskScore}</b></p>
          </motion.div>
          ...
          {/* Conditional Precautions Segment */}
          {hasHeartDisease ? (
